...
for i=1:length(patients)
    data{i} = load([working_dir,'/data_',patients{i},'.mat']);
    ...
    for c=1:length(features{i}.locations)
        if(data{i}.necro_seg(features{i}.locations(c))==1)
            features{i}.labels(c)=3;
        elseif(data{i}.tumor_seg(features{i}.locations(c))==1)
            features{i}.labels(c)=1;
        elseif(data{i}.vasc_seg(features{i}.locations(c))==1)
...
%     features{i} = generate_feature_array(data{i}); %returns only labels
    ...
    f = features{i};
    save([working_dir,'/init_features_',patients{i},'.mat'], 'f');
end
